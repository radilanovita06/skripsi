...
def read_uploaded_file(uploaded_file) -> pd.DataFrame:
    ...
    try:
        ...
        raise RuntimeError(
            "openpyxl belum terbaca di environment. Pastikan requirements.txt berisi "
            "openpyxl==3.1.5 lalu reboot/redeploy aplikasi."
        ) from exc
...
# =========================================================
    ...
    with c2:
        st.download_button(
            ...
            data=to_xlsx(template),
            file_name="Template_Monitoring_Anggaran.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            use_container_width=True
        )

    uploaded_file = st.file_uploader("Upload CSV atau XLSX", type=["csv", "xlsx"])
